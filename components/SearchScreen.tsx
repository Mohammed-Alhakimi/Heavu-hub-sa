...
const SearchScreen: React.FC<SearchScreenProps> = ({ onListingClick }) => {
    ...
    <div className="flex-1 flex flex-col lg:flex-row max-w-[1600px] mx-auto w-full">
      ...
      <aside className="w-full lg:w-80 lg:min-w-80 p-6 lg:border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-surface-dark lg:h-[calc(100vh-65px)] lg:sticky lg:top-[65px] overflow-y-auto custom-scrollbar flex flex-col gap-8 shrink-0 transition-colors">
        <div className="flex items-center justify-between lg:hidden mb-4">
          ...
          <button className="text-primary font-medium text-sm">Reset</button>
        </div>
        ...
        <div className="space-y-3">
          <div className="flex justify-between items-center">
            ...
            <span className="text-xs text-primary cursor-pointer hover:underline">Use my location</span>
          </div>
        ...
        <div className="space-y-3 pt-4 border-t border-slate-100 dark:border-slate-800">
          ...
          <div className="flex items-center gap-2">
            <div className="relative flex-1">
              ...
              <input className="w-full pl-7 pr-2 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm focus:border-primary focus:ring-0 outline-none" placeholder="Min" type="number"/>
            </div>
            <span className="text-slate-400">-</span>
            <div className="relative flex-1">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-xs font-bold">$</span>
              <input className="w-full pl-7 pr-2 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm focus:border-primary focus:ring-0 outline-none" placeholder="Max" type="number"/>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <div className="pt-8 pb-6 px-6 lg:px-10 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-surface-dark sticky top-[65px] z-40 shadow-[0_4px_20px_-10px_rgba(0,0,0,0.05)] transition-colors">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-6">
            <div>
              <h1 className="text-2xl md:text-3xl font-black text-slate-900 dark:text-white tracking-tight mb-2">Find equipment for the job.</h1>
              <p className="text-slate-500 dark:text-slate-400 text-sm md:text-base">Browse 2,400+ listings available in your area.</p>
            </div>
            ...
            >
              <div className="aspect-[4/3] w-full bg-slate-100 dark:bg-slate-800 relative overflow-hidden">
                <div className="absolute top-3 left-3 flex flex-wrap gap-2 z-10">
                  {listing.forSale && <span className="px-2.5 py-1 bg-primary text-white text-[10px] font-bold uppercase tracking-wider rounded shadow-sm">For Sale</span>}
                  {listing.forRent && <span className="px-2.5 py-1 bg-secondary text-white text-[10px] font-bold uppercase tracking-wider rounded shadow-sm">For Rent</span>}
                  {listing.isAvailable && (
                    <span className="px-2.5 py-1 bg-success text-white text-[10px] font-bold uppercase tracking-wider rounded shadow-sm flex items-center gap-1">
                      <span className="material-symbols-outlined text-[12px]">bolt</span> Available
                    </span>
                  )}
                </div>
              ...
              <div className="p-5 flex flex-col flex-1">
                <div className="mb-3">
                  <h3 className="text-lg font-bold text-slate-900 dark:text-white line-clamp-1 group-hover:text-primary transition-colors">{listing.name}</h3>
                  <div className="text-xs text-slate-500 dark:text-slate-400 mt-1 flex items-center gap-2">
                    <span className="flex items-center gap-1"><span className="material-symbols-outlined text-[14px]">schedule</span> {listing.hours ? `${listing.hours.toLocaleString()} hrs` : listing.category}</span>
                    <span className="w-1 h-1 bg-slate-300 rounded-full"></span>
                    <span className="flex items-center gap-1"><span className="material-symbols-outlined text-[14px]">location_on</span> {listing.location}</span>
                  </div>
                </div>
                ...
                <div className="mt-auto space-y-3 pt-4 border-t border-slate-100 dark:border-slate-800">
                  {listing.forSale && (
                    <div className="flex justify-between items-baseline">
                      ...
                      <span className="text-xl font-bold text-slate-900 dark:text-white">${listing.buyPrice.toLocaleString()}</span>
                    </div>
                  )}
                  {listing.forRent && (
                    <div className="flex justify-between items-baseline text-secondary">
                      <span className="text-sm font-medium">Rent</span>
                      <span className="text-base font-bold">${listing.rentDaily}<span className="text-xs font-normal text-slate-500 ml-1">/ day</span></span>
                    </div>
                  )}
                </div>
